...
const News = () => {
  ...
  // Only trigger animation when scrolling down and component is in view
  const shouldAnimate = isInView && scrollDirection === 'down' && !hasAnimated;

  // Set hasAnimated when animation triggers
  useEffect(() => {
    if (shouldAnimate) {
      setHasAnimated(true);
    }
          ...
          {newsItems.map((item, index) => (
            <SwiperSlide key={item.id}>
              <motion.div
                ...
                className="max-w-md bg-white rounded-xl overflow-hidden hover:shadow-xl transition-all duration-300 group"
              >
                {/* Image Container */}
