...
const JobApplicantsTable = ({ applicants }) => {
  ...
  function downloadResume(driveUrl, fileName = "resume.pdf") {
  try {
            ...
            getSortedApplicants().map((applicant) => {
              ...
              return (
                <tr key={applicant.candidate_id} className="border-b hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-4 py-3 text-sm ">
                    <button onClick={()=>downloadResume(applicant.candidate_resume_link)} className="px-3 py-1 cursor-pointer bg-blue-600 text-white rounded-md hover:bg-blue-700 text-xs font-medium transition-colors inline-block">Download</button>
                  </td>
                </tr>
              );
            })
